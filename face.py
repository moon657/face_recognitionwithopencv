...
subjects = ["YoonA", "Nana","Kris Wu","unable to recog"]
...
def predict(test_img):
    ...
    #返回预测的图像
       ...
       print("the recognition result of %s is Hardly Believable!!!!"%(label_text))
       draw_rectangle(img, rect)
       # 标出预测的name
       draw_text(img, label_text, rect[0], rect[1] - 5)
    elif label[1]>80:
...
test_img1 = cv2.imread("test_data/test2.jpg")
test_img2 = cv2.imread("test_data/test3.jpg")
...
timetoken = end - start
print("it takes {:.3f} to recognise one photo that contains one face".format(timetoken))
